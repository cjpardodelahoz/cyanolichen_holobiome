...
#
echo -e 'EXTRACTION OF GENOMIC SEQUENCES USING HMMER'"\n" 
#
...
mafft=${8} # Tell the program if you want to align a set of sequences to make the HMMER profile. yes/no
fasta=${9} # Location of the fasta file to be aligned if this was specified
user_alignment=${10} # Location of the alignment that the user is providing. If the user do not provide an alignment, write "-"
gene=${11} # The gen to analyze
alphabet=${12} # The type of sequence that will be searched:
#  dna   : input alignment is DNA sequence data
#  rna   : input alignment is RNA sequence data
strand=${13} # Indicate in which strand to look for information:
#	-	:Just look on the "-" strand of DNA. Former named Crick
#	+	:Just look on the "+" strand of DNA. Former named Watson
#	+-	:Look on both strands
best_hits=${14} # Indicate the program if you want to obtain just a specific number of hits regadless of the threshold evaluation. If not, write 0
query=${15} # Path to the assembly/fasta that you want to use as query
query_suff=${16} # The suffix of the query sequence(s) e.g. *.fasta
up_stream=${17} # Give the number of extra upstream nucleotides to get extracted. If you do not want this option, write "0"
down_stream=${18} # Give the number of extra downstream nucleotides to get extracted. If you do not want this option, write "0"
slurm=${19} # Indicate the program if you would like to run the output script in a SLURM based cluster: yes/no
if [ ${slurm} = "yes" ]; 
    then
        ...
        max_jobs=${19} # The maximun number of jobs that the user wants to run in the local computer
fi
	...
	# MAIN SLUMRM SCRIPT
            ...
            then
            	if [ "${mafft}" = "yes" ];
            		then
		            	...
		            	# mafft
						mkdir -p hmmer/${pref}/${sign}{count}/alignment 
		                # hmmer
						mkdir -p hmmer/${pref}/${sign}{count}/nhmmer/${gene}/${alphabet}
						mkdir -p hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}
						mkdir -p hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=${sign}(echo hmmer/"${pref}"/"${sign}{count}"/alignment)
						# hmmer
						nhmmer=${sign}(echo hmmer/"${pref}"/"${sign}{count}"/nhmmer/"${gene}"/"${alphabet}")
						sequences=${sign}(echo hmmer/"${pref}"/"${sign}{count}"/sequences/"${gene}"/"${alphabet}")
						single_hits=${sign}(echo hmmer/"${pref}"/"${sign}{count}"/sequences/"${gene}"/"${alphabet}"/single_hits)
					else
						# hmmer
						mkdir -p hmmer/${pref}/${sign}{count}/nhmmer/${gene}/${alphabet}
						mkdir -p hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}
				...
				if [ "${mafft}" = "yes" ];
            		then
            			# Creating directories
		            	# mafft
						mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/alignment 
		                # hmmer
						mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/nhmmer/${gene}/${alphabet}
						mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}
						mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${sign}{count}"/alignment)
						# hmmer
						nhmmer=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${sign}{count}"/nhmmer/"${gene}"/"${alphabet}")
						sequences=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${sign}{count}"/sequences/"${gene}"/"${alphabet}")
						single_hits=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${sign}{count}"/sequences/"${gene}"/"${alphabet}"/single_hits)
					else
						# hmmer
						mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/nhmmer/${gene}/${alphabet}
						mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}
		...
		if [ "${number_hits}" = "+" ]; 
			then
				...
				${sign}nhmmer/${sign}{count}_${gene}.hmm ${query}/${sign}{count}${query_suff}
		elif [ "${strand}" = "-" ];
			then
		...
		if [ "${sign}{number_hits}" -eq 0 ]; then
			...
			cat ${sign}nhmmer/${sign}{count}_${gene}.tbl | grep "${sign}{alignment_query}" | while read line;
			...
			if [ "${best_hits}" -gt 0 ]; then
	        	...
	        	if [ "${out_dir}" = "." ]; then
	        		# Creating directories
					mkdir -p hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}/best_hits
					#Defining variables on the directories locations
					best_hits=${sign}(echo hmmer/"${pref}"/"${sign}{count}"/sequences/"${gene}"/"${alphabet}"/best_hits)
				else
	        		# Creating directories
					mkdir -p ${out_dir}/hmmer/${pref}/${sign}{count}/sequences/${gene}/${alphabet}/best_hits
					#Defining variables on the directories locations
					best_hits=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${sign}{count}"/sequences/"${gene}"/"${alphabet}"/best_hits)
				fi
            ...
            then
            	if [ "${mafft}" = "yes" ];
            		then
            			# Creating directories
		            	# mafft
						mkdir -p hmmer/${pref}/${samp}/alignment 
		                # hmmer
						mkdir -p hmmer/${pref}/${samp}/nhmmer/${gene}/${alphabet}
						mkdir -p hmmer/${pref}/${samp}/sequences/${gene}/${alphabet}

						#Defining variables on the directories locations
						# mafft
						alignment=${sign}(echo hmmer/"${pref}"/"${samp}"/alignment)
						# hmmer
						nhmmer=${sign}(echo hmmer/"${pref}"/"${samp}"/nhmmer/"${gene}"/"${alphabet}")
						sequences=${sign}(echo hmmer/"${pref}"/"${samp}"/sequences/"${gene}"/"${alphabet}")
					else
						# hmmer
						mkdir -p hmmer/${pref}/${samp}/nhmmer/${gene}/${alphabet}
						mkdir -p hmmer/${pref}/${samp}/sequences/${gene}/${alphabet}
				...
				if [ "${mafft}" = "yes" ];
            		then
            			# Creating directories
		            	# mafft
						mkdir -p ${out_dir}/hmmer/${pref}/${samp}/alignment 
		                # hmmer
						mkdir -p ${out_dir}/hmmer/${pref}/${samp}/nhmmer/${gene}/${alphabet}
						mkdir -p ${out_dir}/hmmer/${pref}/${samp}/sequences/${gene}/${alphabet}

						#Defining variables on the directories locations
						# mafft
						alignment=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${samp}"/alignment)
						# hmmer
						nhmmer=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${samp}"/nhmmer/"${gene}"/"${alphabet}")
						sequences=${sign}(echo "${out_dir}"/hmmer/"${pref}"/"${samp}"/sequences/"${gene}"/"${alphabet}")
					else
						# hmmer
						mkdir -p ${out_dir}/hmmer/${pref}/${samp}/nhmmer/${gene}/${alphabet}
						mkdir -p ${out_dir}/hmmer/${pref}/${samp}/sequences/${gene}/${alphabet}
