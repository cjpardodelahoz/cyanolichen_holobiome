			...
			then
				if [ "yes" = "yes" ];
					then
						...
						busco -i /hpc/group/bio1/cyanolichen_holobiome/analyses/illumina/binning/vamb/8066_thalli/8066_thalli_bins/${count}.fna -l - -o ${count}  --augustus -m geno --out_path ${busco_out} --cpu 8 --offline --download_path /hpc/group/bio1/diego/programs/busco/busco_downloads  
					else
						busco -i /hpc/group/bio1/cyanolichen_holobiome/analyses/illumina/binning/vamb/8066_thalli/8066_thalli_bins/${count}.fna -l - -o ${count}  --augustus -m geno --out_path ${busco_out} --cpu 8
				fi		
			else
				if [ "yes" = "yes" ];
					then
						# Using BUSCO in offline mode and automated lineage selection
						busco -i /hpc/group/bio1/cyanolichen_holobiome/analyses/illumina/binning/vamb/8066_thalli/8066_thalli_bins/${count}.fna --auto-lineage -o ${count}  -m geno --out_path ${busco_out} --cpu 8 --offline --download_path /hpc/group/bio1/diego/programs/busco/busco_downloads  
					else
						busco -i /hpc/group/bio1/cyanolichen_holobiome/analyses/illumina/binning/vamb/8066_thalli/8066_thalli_bins/${count}.fna --auto-lineage -o ${count}  -m geno --out_path ${busco_out} --cpu 8
				fi
		fi
