...
#SBATCH --output=/work/dg304/dala/cyanolichen_holobiome/logs/illumina/taxonomy/8066_taxonomic_assignation_kraken_%A_%a.out # Indicate a path where a file with the output information will be created. That directory must already exist
#SBATCH --error=/work/dg304/dala/cyanolichen_holobiome/logs/illumina/taxonomy/8066_taxonomic_assignation_kraken_%A_%a.err # Indicate a path where a file with the error information will be created. That directory must already exist
#SBATCH --partition=common # Partition to be used to run the script
        
        # BUILDING THE VARIABLE FOR THE ARRAY COUNTER
        count=$(cat /hpc/group/bio1/cyanolichen_holobiome/documents/sample_names/8066_sample_names.txt | sort | uniq | sed -n ${SLURM_ARRAY_TASK_ID}p)
        ...
        if [ "/work/dg304/dala/cyanolichen_holobiome/analyses/illumina" = "." ]; 
            then
                ...
                # sequences
                mkdir -p /work/dg304/dala/cyanolichen_holobiome/analyses/illumina/taxonomy/sequences
                # kraken
                mkdir -p /work/dg304/dala/cyanolichen_holobiome/analyses/illumina/taxonomy/kraken/krakens
                mkdir -p /work/dg304/dala/cyanolichen_holobiome/analyses/illumina/taxonomy/kraken/reports
                # bracken
                mkdir -p /work/dg304/dala/cyanolichen_holobiome/analyses/illumina/taxonomy/bracken/brackens
                mkdir -p /work/dg304/dala/cyanolichen_holobiome/analyses/illumina/taxonomy/bracken/reports
        ...
        # KRAKEN
        kraken2 --db /hpc/group/bio1/diego/programs/kraken2/01172023 --threads 16 --paired /hpc/group/bio1/cyanolichen_holobiome/analyses/illumina/reads/${count}/${count}_R1_all.fastq.gz         /hpc/group/bio1/cyanolichen_holobiome/analyses/illumina/reads/${count}/${count}_R2_all.fastq.gz         --output ${krakens}/${count}.kraken         --report ${k_reports}/${count}.report
