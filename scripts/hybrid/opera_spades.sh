...
#SBATCH --output=logs/hybrid/opera_spades_%A_%a.out
#SBATCH --error=logs/hybrid/opera_spades_%A_%a.err
...
mkdir -p analyses/hybrid/assemblies/opera_spades/${sample}
# Unzip long reads for analysis
gunzip -c analyses/ont/reads/${sample}_trimmed.fastq.gz > analyses/ont/reads/${sample}_trimmed.fastq
...
perl ${opera_path}/OPERA-MS.pl \
  ...
  --long-read analyses/ont/reads/${sample}_trimmed.fastq \
  --contig-file analyses/illumina/assemblies/${sample}/contigs.fasta \
  --out-dir analyses/hybrid/assemblies/opera_spades/${sample} \
  --genome-db databases/opera/peltnos \
  --num-processors 16
# Remove unzipped long reads
rm analyses/ont/reads/${sample}_trimmed.fastq